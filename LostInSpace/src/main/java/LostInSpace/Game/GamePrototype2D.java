 ...
 *
 */
public class GamePrototype2D extends Frame implements KeyListener, LevelEvent {
	private Game game;

	private int actualGameSize = 3;
	...
	GamePrototype2D() {
		...
		game = new Game();
		game.setLevel(Level.getLevel(actualGameSize, 2));
		game.AddLevelEventListener(this);
		setSize(1600, 800);
		setVisible(true);
		addKeyListener(this);
	...
	private void drawNode(Graphics2D g2d, Node node, int playerDirection, int x0, int y0, int depth, int top, int left,
			int right) {
		...
		if (!node.isEdgePresent(topDirection))
			drawTopWall(g2d, x0, y0, topDirection);
		if (!node.isEdgePresent(rightDirection))
			drawRightWall(g2d, x0, y0, rightDirection);
		if (!node.isEdgePresent(bottomDirection))
			drawBottomWall(g2d, x0, y0, bottomDirection);
	...
	private void drawLevelGraph(Graphics2D g2d, Node firstNode) {
		...
		for (Node node : accessibleNodes) {
			...
			for (Long key : node.edges.keySet()) {
				...
				if ((key == game.playerDirection && node.equals(game.playerNode))
						|| (((key + 2) % 4) == game.playerDirection && neighbour.equals(game.playerNode))) {
					// If the current node is the "forward movement node", then
					// paint it red.
	...
	public void keyPressed(KeyEvent arg0) {
		...
		// The node of the player or the "forward movement edge" could have
		// changed.
		this.repaint();
	...
	/**
	 * The exit node has been reached. The next level must be triggered.
	 * 
	 * This method is called by a callback mechanism in the Game class instance.
	 */
	@Override
